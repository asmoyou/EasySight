...
class UsersApi {
  ...
  getUserList(params: UserListParams = {}) {
    return request.get<ApiResponse<UserListResponse>>('/v1/users/', { params })
  }
  ...
  deleteUser(id: number) {
    return request.delete<ApiResponse<{ message: string }>>(`/v1/users/${id}`)
  }

  // 重置用户密码
  resetUserPassword(id: number, data: PasswordResetForm) {
    return request.post<ApiResponse<{ message: string }>>(`/v1/users/${id}/reset-password`, data)
  }

  // 获取用户统计信息
  getUserStats() {
    return request.get<ApiResponse<UserStats>>('/v1/users/stats/overview')
  }

  // 批量删除用户
  batchDeleteUsers(userIds: number[]) {
    return request.post<ApiResponse<{ message: string }>>('/v1/users/batch-delete/', { user_ids: userIds })
  }

  // 批量更新用户状态
  batchUpdateUserStatus(userIds: number[], isActive: boolean) {
    return request.post<ApiResponse<{ message: string }>>('/v1/users/batch-update-status/', {
      user_ids: userIds,
      is_active: isActive
    })
  }
