"""add_missing_algorithm_fields

Revision ID: a9c69ffb6efd
Revises: 56f3ac08cdef
Create Date: 2025-07-22 10:51:17.581522
...
revision = 'a9c69ffb6efd'
down_revision = '56f3ac08cdef'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.add_column('ai_algorithms', sa.Column('input_format', sa.JSON(), nullable=True, comment='输入格式'))
    op.add_column('ai_algorithms', sa.Column('output_format', sa.JSON(), nullable=True, comment='输出格式'))
    op.add_column('ai_algorithms', sa.Column('performance_metrics', sa.JSON(), nullable=True, comment='性能指标'))
    op.add_column('ai_algorithms', sa.Column('resource_requirements', sa.JSON(), nullable=True, comment='资源需求'))
    op.add_column('ai_algorithms', sa.Column('supported_platforms', sa.JSON(), nullable=True, comment='支持的平台'))
    # ### end Alembic commands ###


def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.drop_column('ai_algorithms', 'supported_platforms')
    op.drop_column('ai_algorithms', 'resource_requirements')
    op.drop_column('ai_algorithms', 'performance_metrics')
    op.drop_column('ai_algorithms', 'output_format')
    op.drop_column('ai_algorithms', 'input_format')
