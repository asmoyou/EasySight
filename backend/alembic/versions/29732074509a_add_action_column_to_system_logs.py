"""Add action column to system_logs
...
revision = '29732074509a'
down_revision = '1c2bdb569923'
branch_labels = None
depends_on = None
...
def upgrade() -> None:
    ...
    op.drop_table('roles')
    op.add_column('system_logs', sa.Column('action', sa.String(length=100), nullable=True, comment='操作动作'))
    op.add_column('system_logs', sa.Column('extra_data', sa.JSON(), nullable=True, comment='额外数据'))
...
def downgrade() -> None:
    # ### commands auto generated by Alembic - please adjust! ###
    op.add_column('users', sa.Column('roles', postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True, comment='用户角色列表'))
    op.alter_column('user_login_logs', 'user_id',
               existing_type=sa.INTEGER(),
               nullable=True,
               existing_comment='用户ID')
    op.add_column('system_logs', sa.Column('details', postgresql.JSON(astext_type=sa.Text()), autoincrement=False, nullable=True, comment='详细信息'))
    op.drop_column('system_logs', 'extra_data')
    op.drop_column('system_logs', 'action')
